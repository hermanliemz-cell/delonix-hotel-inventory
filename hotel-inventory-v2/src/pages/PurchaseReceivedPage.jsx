...
function PurchaseReceivedPage() {
  ...
  async function generateGRNumber() {
    ...
    const prefix = `GR-${selectedOrg.code}-${yy}${mm}`;
    const { data } = await supabase.from('purchase_received').select('gr_number').eq('organization_id', selectedOrg.id).like('gr_number', prefix + '%').order('gr_number', { ascending: false }).limit(1);
    let nextNum = 1;
    if (data && data.length > 0) {
      const lastNum = parseInt(data[0].gr_number.slice(prefix.length), 10);
