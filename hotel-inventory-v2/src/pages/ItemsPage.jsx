...
function ItemsPage() {
  ...
  async function handleDeleteItem(item) {
    try {
      ...
      // Check room_items (standard room config) — these will be cleaned up on delete
      const { count: riCount } = await supabase.from('room_items').select('id', { count: 'exact', head: true }).eq('item_id', item.id);

      // No relations — confirm delete
      const confirmMsg = riCount > 0
        ? `Hapus item "${item.name}" (${item.code})?\n\nItem ini memiliki ${riCount} konfigurasi room standar yang juga akan dihapus.`
        : `Hapus item "${item.name}" (${item.code})?`;
      if (!(await showConfirm(confirmMsg, { variant: 'danger' }))) return;

      // Clean up related data that can be safely removed
      await supabase.from('stock_balance').delete().eq('item_id', item.id).eq('quantity', 0);
      if (riCount > 0) await supabase.from('room_items').delete().eq('item_id', item.id);
